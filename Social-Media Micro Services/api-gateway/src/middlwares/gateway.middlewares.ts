...
const GatewayMiddlewares = async (app: Application) => {
  ...
  app.use(
    cors({
      credentials: true,
      origin: EnvConfig.CROSS_ORIGINS.filter((origin): origin is string => Boolean(origin)),
  ...
  const rateLimiter = new RateLimiterRedis({
    ...
    keyPrefix: 'gateway-service:rate_limiting_middleware',
    points: 500,
    duration: 60, // 60s = 1 minute
  })
  ...
  app.use(
    compression({
      ...
      threshold: 1000,
      filter: (req, res) => {
        if (req.headers['x-no-compression'] || !req.acceptsEncodings('gzip', 'br')) {
          return false
        }
...
export default GatewayMiddlewares
