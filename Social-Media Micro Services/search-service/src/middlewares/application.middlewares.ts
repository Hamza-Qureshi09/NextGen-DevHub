...
const ApplicationMiddlewares = async (app: Application) => {
  ...
  const rateLimiter = new RateLimiterRedis({
    ...
    ...redisRateLimitConfigs.search,
  });
  app.use((req: Request, res: Response, next: NextFunction) => {
    const clientIP = req.headers["x-forwarded-for"] || req.ip; // Get IP from proxy or default
    rateLimiter
      .consume(clientIP as string)
      .then(() => next())
      .catch(() => {
        logger.warn(`Rate limit exceeded for IP: ${req.ip}`);
        return res
          .status(Responces.TOO_MANY_RQSTS)
          .json({ message: Messages.TOO_MANY_RQSTS });
      });
  });
