...
export const createRateLimiter = (config: { windowMs: number; max: number; message: string; prefix?: string }) => {
  return rateLimit({
    ...
    legacyHeaders: false,
    handler: (req, res) => {
      logger.info(`Rate limit exceeded for IP: ${req.ip} on ${config.prefix || 'unknown'} routes`)
    ...
    store: new RedisStore({
      ...
      prefix: config.prefix || 'default:auth:', // Adding prefix to distinguish b/w different limiters in Redis
    }),
  })
}
